...
def friends(request):
    ...
    if request.method == 'POST' and request.POST.get("send_requedst"):
        receivers = request.POST.getlist("send_requests")
        for receiver in receivers:
            receiver_profile = Profile.objects.get(id=receiver)
            Relationship.object.create(sender=user_profile,receiver=receiver_profile,status='sent')
    ...
    context = {'user_freidns_profiles': user_friends_profiles, 'user_relationships': user_relationships, 'all_profiles': all_profiles,
                    'request_received_profiles': request_received_profiles}

    return render(request, 'FeedApp/freinds.html',context)